...
This is just a simple substitution cipher (and we're given the lookup tables already) so reversing this is quite trivial (that is, we just do the steps but flip the operations). My script is as follows:
...
However, this yielded some gibberish that was clearly not the flag. I pondered for a moment on the peculiarity of this scenario, given there was only one ciphertext. But then I remembered the strange comments at the top of `convert2.py`:
